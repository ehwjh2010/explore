...
Skill applicability is the authorization signal: when a task meets the Explore conditions, or when the user invokes `$explore`, the skill treats that as an explicit request to delegate read-only reconnaissance to explorer subagents. Subagent reconnaissance is mandatory before the main agent does detailed local reads or edits. The main agent should split the question, send complementary read-only slices to explorer subagents, and synthesize their summaries before doing detailed local reads.

There is no local fallback reconnaissance mode. If explorer subagents cannot be spawned, Explore stops and reports the blocker instead of scanning the codebase locally.
...
- Stop instead of using local fallback if explorer subagents cannot be spawned.
...
This skill requires explorer subagent spawning when it applies; no extra user phrase such as `subagents`, `delegate`, or `parallel agents` is required by the skill. Whether subagents can actually be spawned is determined by the active Codex runtime and available tools.
