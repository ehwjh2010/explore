...
Two explorer subagents checked the authentication path and nearby tests. The active password-login path appears to be `src/routes/login.ts -> src/auth/password.ts -> src/auth/session.ts`. An older implementation under `src/legacy/auth.ts` is imported only by migration tests and should not be changed for the requested behavior.
...
| `src/legacy/auth.ts` | Legacy implementation | No | Only referenced by migration tests; not active for login route | auth-path explorer |
